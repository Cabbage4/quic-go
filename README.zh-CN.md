...
本仓是从零开始、面向学习与教学的实现，**未做任何性能优化**。下列数据是基线而非目标。所有测量均在回环（`127.0.0.1`）上进行，反映的是纯协议栈开销、不含网络 RTT。
...
- **每请求延迟随 N 线性增长**（N 从 300→1000，延迟 3.2→9.6 ms）：可扩展性问题。主因是累计 ACK 区间随已发包数增长，每收到一个 ACK 都要把"已确认包集合"重新物化并整段遍历（每个 ACK 一次 O(N) 扫描；已加 2^20 上限以防内存失控）。已发帧跟踪 map 也随 N 增长。
- **大块传输（8 MiB echo）会死锁** —— 无论"先写后读"还是"并发读"，15s 内都未完成。这说明大块传输下流控/写阻塞存在问题，与已知缺陷（连接级流控窗口在某些路径下未正确传播）一致。
- **ACK 频率实际为 1**：每收一个 ack-eliciting 包就单独回一个 ACK 包（无 ACK 合并、无延迟 ACK），所以每个请求在链路上约 ~10 个包。
- 类 NewReno 拥塞控制，无 pacing。

### 结论

吞吐比生产级栈低数个数量级（参考 [quic-go](https://github.com/quic-go/quic-go) 可达多 Gbit/s、数万 req/s）。对一个未优化、单文件单职责的学习实现而言属预期。按优先级，提升空间最大的是：

1. **ACK 合并 / 延迟 ACK** —— 把每请求 ~10 个包降到 ~2-3。
2. **修复连接级流控窗口增长**，使大块传输不再死锁。
3. **`ParseAckFrame` 惰性区间遍历**（按区间迭代而非物化整段切片），消除每个 ACK 的 O(N) 开销。
4. Pacing 与更现代的拥塞控制（如 BBR）。
